...
abstract class QuickUsbPlatform extends PlatformInterface {
  ...
  ///
  /// Patched (nikon_z_control fork): added to let PTP-USB transports
  /// wipe a residual endpoint halt left behind by a previously crashed
  /// session — without this the first bulkTransferOut after (re)claim
  /// can hang until timeout. Default implementation is a no-op; the
  /// Android platform overrides it with a real CLEAR_FEATURE control
  /// transfer.
  Future<bool> clearHalt(UsbEndpoint endpoint) async => true;
}
