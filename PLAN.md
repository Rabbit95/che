...
## Progress 快照（更新于 2026-08-18，M3 完成 + USB transport 硬化）
...
| **M6b USB (iOS)** | 🔴 未开始 | `IccTransport` 空壳 stub | ICCameraDevice Pigeon 桥、Wired Accessories 权限、iOS 18+ 回归适配 |
...
1. **M4 真机验证**：主线代码已就绪，需要真机（Wi-Fi 优先，USB 也应能工作）跑一次拿实测 fps + 确认 LV header 偏移量在当前固件下正确；如偏移不对，`LiveViewFrameCodec._parseHeader` 是唯一需要按机型调的地方
2. **M5 Gallery/Transfer 对接**（首屏可用性最大）：Gallery 用 `getObjectHandles + getThumb`；Transfer 用 `downloadObject` + `photo_manager`
3. **M1 mDNS 完整化**：把 `discoveryProvider` 里的 stub 换成 bonsoir 真实流（原计划里跳过的部分）
4. **M7 重连状态机**：断网/挂起/后台切换的 keepalive + 自动重连
